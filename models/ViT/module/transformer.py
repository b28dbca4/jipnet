...
class LoFTREncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, attention='linear'):
        ...
        # NOTE (paper vs code): paper Eq.3 describes standard softmax attention,
        # but the released model was trained with ELU-based linear attention
        # (Katharopoulos et al.). Changing this would break pretrained checkpoints.
        self.attention = LinearAttention(
        ) if attention == 'linear' else FullAttention()
        self.merge = nn.Linear(d_model, d_model, bias=False)

        # feed-forward network
        # NOTE (paper vs code): paper cites ViT (GELU FFN), but trained with ReLU.
        # Changing to GELU would break pretrained checkpoints.
